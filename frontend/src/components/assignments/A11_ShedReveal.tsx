...
interface DragItem { index: number; letter: string }

function DraggableLetter({
  letter, index, moveLetter, sparkle,
}: {
  letter: string; index: number; moveLetter: (from: number, to: number) => void; sparkle: boolean
}) {
...
function ShedRevealInner() {
  ...
  // Start with shuffled version of the word code letters
  const [letters, setLetters] = useState<string[]>(() => {
    const chars = wordCode.split('')
    // Shuffle so it doesn't start in correct order
    const target = wordCode
    const p = [...chars]
    do {
  ...
  const moveLetter = useCallback((from: number, to: number) => {
    setLetters((prev) => {
      const next = [...prev]
    ...
    return (
      <div className="flex flex-col items-center justify-center gap-8 py-12 text-center animate-pop">
        ...
        <p className="text-lg text-text-secondary max-w-sm">
          You cracked the word code! Lock 2 is now open!
        </p>
        ...
        <div className="flex flex-col items-center gap-2">
          ...
          <div className="flex gap-2">
            {wordCode.split('').map((l, i) => (
              <div key={i} className="w-16 h-20 flex items-center justify-center text-3xl font-bold bg-yellow-accent text-blue-primary rounded-xl border-3 border-blue-primary shadow-lg">
                {l}
              </div>
            ))}
          </div>
      ...
      <div className="text-center space-y-3">
        ...
        <div className="flex items-center justify-center gap-2">
          {wordCode.split('').sort().map((l, i) => (
            <div key={i} className="w-10 h-12 flex items-center justify-center text-xl font-bold font-mono bg-yellow-tint border-2 border-yellow-accent text-blue-primary rounded-lg">
              {l}
            </div>
          ))}
        </div>
      ...
      <div className="bg-blue-tint rounded-2xl p-5 flex flex-col items-center gap-4 border-2 border-blue-light/30">
        ...
        <div className="flex gap-3">
          {letters.map((letter, index) => (
            <div key={`${letter}-${index}`} ref={(el) => { letterRefs.current[index] = el }}>
              <DraggableLetter
                letter={letter}
                index={index}
        ...
        <div className="flex items-center gap-3">
          ...
          <span className="text-xl font-bold font-mono text-blue-primary tracking-widest">
            {letters.join('')}
          </span>
        </div>
